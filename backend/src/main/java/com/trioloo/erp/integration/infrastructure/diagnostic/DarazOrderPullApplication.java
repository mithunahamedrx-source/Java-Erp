...
/**
 * The explicitly scoped context the order pull probe runs in.
 *
 * <p>🔴 IT SCANS NOTHING AND NAMES ITS DEPENDENCIES, for the reason
 * {@code DarazListingShapeApplication} already records: {@code WebApplicationType.NONE} stops
 * Tomcat but NOT component scanning, and a scanned start pulls in the servlet security graph this
 * command has no use for — which is how the first such command died in production.
 *
 * <p>🔴 THE IMPORT LIST IS THE PROBE'S BLAST RADIUS, AND IT IS DELIBERATELY SHORT. No order writer,
 * no listing writer, no operation recorder, no channel adapter, no scheduler and no controller is
 * named here — so there is no bean in this context that COULD write an Order, a Listing, an
 * Inventory position or a Payment, whatever the probe were asked to do.
 *
 * <p>⚠ Read-only repositories reach it through {@code @EnableJpaRepositories}; the probe uses two
 * of them — channel identity and connection state — and calls only finders.
 *
...
@Import({
        ...
        DarazOrderPullProbe.class,
        DarazOrderPullRunner.class,
})
public class DarazOrderPullApplication {
}
