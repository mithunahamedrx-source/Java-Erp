...
**Version:** 1.82.0 · **Amended:** 2026-08-24 (**`DOC-097` — [`ORDER_MODULE_PAGE_CONTRACT.md`](ORDER_MODULE_PAGE_CONTRACT.md) v1.0.0 registered as a WORKING RECORD, not canonical: it consolidates `ORDERS_SCREEN_CONTRACT.md` into a per-PAGE build sheet and supplies the Claude Design brief, issues no rule, claims no prefix and yields to the contract on any conflict. 🔴 THE FINDING THAT CHANGES WHAT GETS DESIGNED: `OSC-020.a` makes `FRAME 03`–`FRAME 09` PANELS of the `FRAME 02` surface, so the Order module is FOUR pages and not nine — designing nine would build a structure the contract forbids. ⚠ Two LIVE surfaces have no frame number: manual capture (`PRM-093`, `UX-151`) and the invoice printable (`OSC-059`); recorded as OWED rather than numbered there**) · **Amended:** 2026-08-24 (**COURIER AND ORDER-CREATION CAPABILITY CODES RATIFIED — `PERMISSION_ARCHITECTURE.md` v1.15.0 `PRM-092`/`PRM-093`, migration `V20`. ✅ `delivery.shipment.book`, `.track`, `.cancel` and `payment.courier-remittance.view` CLOSE `GAP-138.g`: `DLV §22` had required every dispatch permissioned and attributable since ratification with NO code to enforce it. 🔴 The remittance code's module segment is `payment`, NOT `delivery` — the product owner's proposal said otherwise, but `PAY-022` and `DLV §23` both place `E-042` with Payment and `PRM-089.a` requires the OWNING module to name it; the correction is a derivation, not a change of intent. ✅ `PRM-093` `order.order.create` — a manual order starts at `PENDING_VERIFICATION`, the same state an imported one arrives in, and CREATION IS NOT CONFIRMATION. 🔴 `V20` seeds all five with ZERO holders (`PRM-003`, `PRM-081.b`). ⚠ A pre-existing test hazard found and worked around: `AccessFixtures.clear()` issues `DELETE FROM permission` and the shared test database keeps it between classes, so any assertion against seeded permission rows is ORDER-DEPENDENT; the seed test pins the immutable migration file instead. `DOC-096` — [`ORDER_MODULE_ROADMAP.md`](ORDER_MODULE_ROADMAP.md) registered as a working record, fixing the per-phase discipline: page contract → API mismatch check → design → build → report**) · **Amended:** 2026-08-24 (**`DOC-095` — [`STEADFAST_PROVIDER_CONTRACT.md`](STEADFAST_PROVIDER_CONTRACT.md) v1.0.0 registered, owning `STF-`. ⚠ NOT canonical architecture — a third-party protocol reference, the second in the corpus after `DARAZ_PROVIDER_CONTRACT.md`. `GAP-138` REGISTERED and PARTIALLY ADDRESSED: `DLV-013` has ratified Steadfast as the sole courier since ratification and NO document ever recorded the protocol. ✅ Base host is `portal.packzy.com/api/v1`, NOT the brand domain; auth is two STATIC headers with no expiry. 🔴 Three runtime hazards recorded: the envelope carries THREE different `status` shapes across three endpoints; `401 Unauthorized Access` conflates *not found*, *not yours* and *not authenticated* and arrives as PLAIN TEXT, so it must never be read as a credential failure; and a leaked static key persists until a human rotates it. ✅ The COD remittance feed CORROBORATES `BR-035`/`SMA-079` — `created_at`/`ready_at`/`paid_at` are the three moments `BD-438` described, and `status_label: paid` is the courier's claim and never Trioloo's receipt, so no adapter may auto-advance `SM-5` from it. 🔴 `create_order` was NOT CALLED — booking dispatches a real courier — and the duplicate-`invoice` behaviour is named as the most important unknown, because a silent double booking would violate `BR-023` at the courier. 🔴 No `delivery_status` value observed, so NO `SM-4` mapping is written. Nothing implemented, no business rule created, no secret recorded**) · **Amended:** 2026-08-24 (**ORDERS PAGE-HEADER ACTIONS — `ORDERS_SCREEN_CONTRACT.md` v1.9.0 `OSC-058`. ⚠ THE OWNER ASKED FOR THREE ACTIONS; ONE IS BUILT AND TWO ARE BLOCKED ON CANONICAL GROUNDS. 🔴 `Create Order` — BLOCKED — MISSING CANONICAL BUSINESS RULE: `PRM-091` ratifies two Order capability codes and states NEITHER grants Order mutation; `PRM-089.b` is a spelling rule and not a generator, so no create permission may be minted. 🔴 `Print` — BLOCKED: `PRN-023` sources the printable from an `E-039` record whose content `INV-39.2` requires snapshotted, and none exists — the invoice NUMBER was issued, the DOCUMENT was not. ✅ Both render disabled with the reason in VISIBLE text. ✅ `Export` is built in full under `RPT-046`/`UX-044`: scope is the selection or the ACTIVE RESULT SET, never the visible page; buyer-supplied text is neutralised against spreadsheet formula injection; unknown economics are written as the WORD because an empty cell sums as zero. ✅ Page size five; `Select all` takes the current page and a selection SURVIVES paging, amending `OSC-057.c.i`**) · **Amended:** 2026-08-24 (**THE TRIOLOO-ISSUED INVOICE NUMBER — `ORDERS_SCREEN_CONTRACT.md` v1.8.0 `OSC-057`, migration `V19`, `GAP_ANALYSIS.md` `GAP-035` PARTIALLY ADDRESSED. ⚠ THE PRODUCT OWNER SUPPLIED A BUSINESS RULE `GAP-035` RECORDED AS MISSING: one sequence, `TR0001` upward, oldest to newest, every existing and future order, none skipped, none ever regenerated. ✅ `BD-443`/`INV-39.1` permit exactly ONE Sales Invoice sequence and prohibit a second; this is that one. ✅ A production dry-run confirmed the backfill yields `TR0001` (2026-05-31) through `TR0158` and that all 158 ordering keys are distinct, so the numbering is deterministic rather than plan-dependent. 🔴 IMMUTABILITY IS ENFORCED BY A TABLE TRIGGER, not by the application, because *never regenerated* must also survive a repair script and a console session. ⚠ The number is issued by a GUARDED UPDATE, not a column `DEFAULT`, which at the `PT2M` cadence would burn ~113,000 sequence values a day. 🔴 REPORTED, NOT RESOLVED — THE NUMBER IS ISSUED AHEAD OF THE DOCUMENT: `E-039`'s snapshotted content (`INV-39.2`) does not exist and 117 of the 158 numbered orders are `CANCELLED`; `GAP-035` stays open on what an invoice CONTAINS and WHEN it is issued. ✅ Order cards gain a selection checkbox with NO bulk bar (`PRM-025`, `GAP-034`)**) · **Amended:** 2026-08-24 (**ORDER CARD AMENDED BY THE PRODUCT OWNER — `ORDERS_SCREEN_CONTRACT.md` v1.7.0 `OSC-056`. ✅ The `SM-5` PAYMENT POSITION is now a chip, and only THREE of its eleven states may ever be rendered: `Payment not due` before delivery, `Payment due` on `DELIVERED`, `Payment unknown` for `RETURNED`/`CLOSED`/untranslated. 🔴 Everything past `DUE` needs an `E-040` Receivable that has been collected, matched or settled, and no such record exists — the same absence `OSC-053` recorded. ⚠ `Payment due` does not assert the buyer has not paid (`BR-035`), and is INFO not SUCCESS because `OM §11.1` calls paid-at-delivery a false statement on every channel Trioloo operates. ✅ The marketplace's word loses its prefix and stays visibly external by GROUPING. 🔴 The authority chip leaves the card, admissible only because `FRAME 02` carries it — and `OSC-056.d.i` REPORTS that `FRAME 02` states it as `API-managed`, the internal terminology `UX-183` forbids; the fix is owed and not made here. ⚠ `More Actions` is rendered with the owner's ratification but NO action behind it, so `OSC-051.b` is only half satisfied and the control says so. ✅ The invoice element sheds its number and becomes the printable-invoice action slot**) · **Amended:** 2026-08-24 (**ORDER CARD BUILT FROM ITS APPROVED DESIGN — `ORDERS_SCREEN_CONTRACT.md` v1.6.0 `OSC-055`. 🔴 The Claude Design project returned `HTTP 404 — project not found`, the same condition `OSC-012` recorded, so the owner supplied the rendered bundle and the specification was TRANSCRIBED from its markup rather than imagined. ✅ Every colour in the design resolved to an existing token — nothing invented; one value with no token is recorded rather than hard-coded (`RULE 15.1`). 🔴 The design's SAMPLE FIGURES ARE REFUSED: Cost, Charges, Received and Margin render UNKNOWN (`INV-32.4`, `BR-033`, `BR-007`, `SYS-034`, the `E-032` defect), the `Not Released` chip is not rendered because `BR-080` withdrew the state, and `More Actions` is not drawn (`OSC-051.b`). ✅ `design-reference/README.md` now REGISTERS its packs, discharging the `OSC-011.c` observation. A SHOP filter is added — `BR-002` makes channel type alone insufficient attribution**) · **Amended:** 2026-08-23 (**ORDERS INGESTION DEPLOYED AND LIVE — `PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.7.0 `DEP-126`: `V17` and `V18` applied behind the `DEP-060` backup gate, 109 orders read from three Daraz shops, 0 failed runs. 🔴 THE PRE-FLIGHT READ CORRECTED A DOCUMENTED FACT — `DEP-125` and `LISTINGS_PAUSE_HANDOFF.md` §4 both stated production was at `V14` with `V15` unapplied, and it was not; `DEP-070.b`'s never-assume rule is what caught it. `DEP-070.a`/`.e` reconciled from `V12` to `V18`. `LISTINGS_PAUSE_HANDOFF.md` v1.1.0 — §6.a's `V15` unknown RESOLVED. ⚠ A fourth shop is excluded throughout because it is `DRAFT`, which `BR-181.a` excludes even when CONNECTED; admitting it is a separate business decision (`BR-181.c`)**) · **Amended:** 2026-08-23 (**ORDERS KPI ROW AND STATUS TABS RATIFIED AND BUILT — `ORDERS_SCREEN_CONTRACT.md` v1.5.0 `OSC-053`/`OSC-054`: the product owner named four summary figures (Total orders · Today's orders · Today's dispatched · Total collectable), closing `GAP-004` for this workspace. 🔴 The shipped KPIs were WITHDRAWN, not defined — `Total Revenue` and `Total Margin` are gone, because `BR-007` and `SYS-034` make a pre-settlement margin confidently wrong and undetectable. The tabs are `SM-1` state names filled by the §4.3 ADAPTER translation (`BR-005`, `DZC-045.c.ii`), so `GAP-017` stays blocked and becomes irrelevant to the tab set rather than closed; `OSC-030.a` gains `Returned`. `GAP_ANALYSIS.md` v2.68.0. ⚠ Two limitations recorded rather than designed around: `Total collectable` subtracts nothing because no receipt or settlement record exists, and `Today's dispatched` counts an ERP OBSERVATION because the provider publishes no dispatch timestamp. 🔴 A `TEC-015` defect found and fixed in the same change — every Orders money field crossed as a JSON number and is now a string. Migration `V17` added; no deploy**) · **Amended:** 2026-08-23 (**ORDERS LIVE UI CLEANUP — `ORDERS_SCREEN_CONTRACT.md` v1.4.0 `OSC-051`: unresolved blocker placeholders, legacy status tabs, disabled future buttons and KPI shells are withheld from the operator-facing read-only Orders UI; `OSC-050` remains the canonical blocked register. Frontend only: no backend, no migration, no deploy in this commit, no Daraz call**) · **Amended:** 2026-08-23 (**ORDERS READ-ONLY UI BUILT — `ORDERS_SCREEN_CONTRACT.md` v1.3.0; `/sales/orders` and `/sales/orders/:id` now render `FRAME 01`/`FRAME 02` read-only surfaces. Frontend only: no backend, no migration, no deploy, no Daraz call**) · **Ratified:** 2026-08-04 · **Amended:** 2026-08-23 (**ORDERS INGESTION MVP DECIDED — `ORDER_MANAGEMENT_ARCHITECTURE.md` v1.21.0 §29 `BR-178`–`BR-183`: backfill in 7-day chunks to a 3-month cap with a boundary probe and stop-on-refusal, a 15-minute CONFIGURABLE cadence whose watermark must OVERLAP and deduplicate by `order_id`, one explicit channel instance per job with scheduler fan-out, `ACTIVE` shops only, no in-job retry, and the webhook explicitly OUT of MVP. `PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.6.0 `DEP-125` — the `V15` position RESOLVED and TAKEN: production sits at `V14` because the deployed jar carries no `V15`, the next deploy applies it deliberately behind the backup gate, and V15-free branches are discontinued. ✅ `ORDERS_SCREEN_CONTRACT.md` v1.2.0 `OSC-060` — the migration bar is LIFTED, still only after the `DEP-031` pre-flight. `GAP_ANALYSIS.md` v2.67.0. 🔴 MVP scope only: no `DRAFT` shop pull, no webhook, no order write, no shipment, no inventory movement, no settlement**) · **Amended:** 2026-08-23 (**FIRST ORDER READ FROM DARAZ — `DARAZ_PROVIDER_CONTRACT.md` v1.13.0 `DZC-057`: `/orders/get` run ONCE against a connected Bangladesh shop and ACCEPTED. ✅ `DZC-050.d` SETTLED — the gateway accepted `limit`, so `limt` is a typo in the provider's own table. 🔴 `DZC-042.d` WIDENED — `_trace_id_` is a general envelope field, seen on reads as well as writes. ✅ `DZC-045.e` CONFIRMED — 32 order fields, all documented — but the table is a CEILING, not a guarantee: `address_updated_at` is documented and was NOT returned. ⚠ The `V15` contradiction is ALSO resolved and was never one: production is at `V14` because the DEPLOYED jar carries `V1`–`V14` and no `V15`; the hazard remains live for any deploy built from `main`. `GAP_ANALYSIS.md` v2.66.0, `GAP-137` still PARTIALLY ADDRESSED — what remains is business, not protocol. 🔴 Nothing imported, nothing written, no migration, no business decision**) · **Amended:** 2026-08-23 (**FIRST ORDERS CODE — `PERMISSION_ARCHITECTURE.md` v1.14.0 `PRM-091` ratifies the two Order MVP capability codes `order.channel-order.view` and `order.channel-order.sync`; neither grants Order mutation, inventory movement, payment/settlement, shipment action or any marketplace write, and one `sync` code covers probe, incremental poll and backfill alike. `ORDERS_SCREEN_CONTRACT.md` v1.1.0 — `OSC-052` DISCHARGED, gating unblocked and no frame built. ⚠ The code shipped is a READ-ONLY diagnostic that creates no schema, no migration, no endpoint and no scheduler**) · **Amended:** 2026-08-23 (**`DARAZ_PROVIDER_CONTRACT.md` v1.12.0 §13 `DZC-051`–`DZC-056` — the DARAZ ORDER NOTIFICATION protocol. A webhook exists and carries order messages, and `trade_order_id` is published as mapping to the API's `order_id`, so pushed and polled reads join on the key `DZC-049.b` already fixed. 🔴 THE DECISIVE FACT: the payload is not the order, and abandonment after twelve retries, abort on lapsed authorisation, and no ordering or at-most-once guarantee each independently force PERIODIC RECONCILIATION — so "webhook instead of polling" is not available and the cadence question is NOT settled. ⚠ `BD-159` partially answered on the provider side only; eleven facts NOT PUBLISHED, including the whole `message_type` enumeration and whether the webhook exists on the BANGLADESH venture. `GAP-137` stays PARTIALLY ADDRESSED, `GAP_ANALYSIS.md` v2.65.0. 🔴 Nothing implemented, no callback endpoint, no subscription, no App Console, no credential, no seller API call**) · **Amended:** 2026-08-23 (**`DARAZ_PROVIDER_CONTRACT.md` v1.11.0 §12 `DZC-043`–`DZC-050` — the DARAZ ORDER READ protocol, rendered from the Daraz-published reference before any implementation. Eight endpoints registered, four reads specified; `order_id` is the external idempotency key; no opaque cursor exists so a checkpoint can only be an OVERLAPPED timestamp watermark. 🔴 Nine protocol facts recorded NOT PUBLISHED, including the retention limit a backfill decision most needs. `GAP-137` moves to PARTIALLY ADDRESSED, `GAP_ANALYSIS.md` v2.64.0. ⚠ Documentation only — nothing implemented, no seller API called, no credential used, and every business question left open**) · **Amended:** 2026-08-23 (**`GAP-137` REGISTERED — the Daraz ORDER API protocol is undocumented while `OM §4.3`/`§7.8`/`EVT-002` already ratify that orders arrive through it. `GAP_ANALYSIS.md` v2.63.0. 🔴 OPEN — NOT ADDRESSED: every official Daraz and Lazada documentation surface returned a JavaScript shell on 2026-08-23, so no protocol was rendered, and NOTHING was written from memory. ⚠ UNREADABLE IS NOT UNPUBLISHED. `DARAZ_PROVIDER_CONTRACT.md` is UNCHANGED at v1.10.0 and gains no `§12`**) · **Amended:** 2026-08-23 (**`DOC-094` — [`ORDERS_SCREEN_CONTRACT.md`](ORDERS_SCREEN_CONTRACT.md) v1.0.0 registered, owning the `OSC-` prefix. The THIRD screen contract in the corpus: it registers the *Trioloo Orders Screens* artifact and its `Order Dashboard` / `Order Detail` artboards as the approved visual authority, records `FRAME 01`–`FRAME 09` with none built, and fixes per frame the required data, allowed actions, refused actions, rule dependencies and owed test coverage. 🔴 Ten blocked elements registered and none decided; no `order.*` permission code is ratified; no migration number may be proposed until production `flyway_schema_history` is read**) · **Amended:** 2026-08-22 (**`DOC-093` — [`LISTINGS_PAUSE_HANDOFF.md`](LISTINGS_PAUSE_HANDOFF.md) v1.0.0 registered as a 📌 STATUS RECORD, not canonical architecture: it claims no rule prefix, issues no rules, changes no status and closes no gap. It records where Listings was left when the Listing View visual redesign was paused for the Order module, and states the `V15` applied/unapplied contradiction rather than resolving it**) · **Amended:** 2026-08-21 (**`LISTINGS_SCREEN_CONTRACT.md` v1.16.0 `LSC-063` — the capability-driven push control on `FRAME 10`; no backend change**) · **Amended:** 2026-08-21 (**FIRST MARKETPLACE WRITE — `PRODUCT_ARCHITECTURE.md` v1.30.0 §39.18 `PRD-205`: `sale_price` and `listing_stock` push-supported on Daraz, every other field local-only. `DARAZ_PROVIDER_CONTRACT.md` v1.10.0 `DZC-042`, `LISTINGS_SCREEN_CONTRACT.md` v1.15.0 `LSC-062`, `GAP_ANALYSIS.md` v2.62.1**) · **Amended:** 2026-08-21 (**`DARAZ_PROVIDER_CONTRACT.md` v1.9.0 `DZC-041` — the controlled same-value price/stock probe, BUILT and NOT RUN; no field became writable**) · **Amended:** 2026-08-21 (**`DARAZ_PROVIDER_CONTRACT.md` v1.8.0 §11 `DZC-033`–`DZC-040` — the listing WRITE protocol recorded from the official reference before any implementation; `GAP_ANALYSIS.md` v2.62.0 `GAP-136`; `LISTINGS_SCREEN_CONTRACT.md` v1.14.1. ⚠ Documentation only — nothing implemented, no seller API called, no field became writable**) · **Amended:** 2026-08-20 (**BUSINESS MODEL — `PRODUCT_ARCHITECTURE.md` v1.29.0 §39.17 `PRD-204`: the operator edits the marketplace listing; intended-vs-reported retained as PERSISTENCE, Accept Marketplace demoted off the ordinary path. `LISTINGS_SCREEN_CONTRACT.md` v1.13.0 `LSC-061`, `GAP_ANALYSIS.md` v2.61.0**) · **Amended:** 2026-08-19 (**`LISTINGS_SCREEN_CONTRACT.md` v1.12.0 `LSC-060` — `FRAME 06` tabbed product view; composition refinement authority recorded, Feature Pack unchanged**) · **Amended:** 2026-08-19 (**`DARAZ_PROVIDER_CONTRACT.md` v1.7.0 §10 `DZC-032` — product review protocol; views/wishlist/cart proven to have no API**) · **Amended:** 2026-08-19 (**`LISTINGS_SCREEN_CONTRACT.md` v1.11.0 `LSC-059` — capability read from the declaring adapter; readable/editable/pushable separated**) · **Amended:** 2026-08-19 (**`LISTINGS_SCREEN_CONTRACT.md` v1.10.0 `LSC-058` — provider markup normalised for display only; nothing stored or compared changes**) · **Amended:** 2026-08-18 (**`LISTINGS_SCREEN_CONTRACT.md` v1.9.0 `LSC-057` — `FRAME 19` inbound half reconciled and `LSC-003` tags closed; frame still PARTIAL, outbound blocked**) · **Amended:** 2026-08-18 (**`LISTINGS_SCREEN_CONTRACT.md` v1.8.0 `LSC-056` — `FRAME 20` implemented against verified backend behaviour; four result figures unavailable, `FRAME 18`/`19` unchanged, `GAP-134` still open**) · **Amended:** 2026-08-18 (**`LISTINGS_SCREEN_CONTRACT.md` v1.7.0 `LSC-055` — `FRAME 10` explains an unauthored Listing; frontend only, no title or business rule changed**) · **Amended:** 2026-08-18 (**`GAP-134` — `discover()` recorded no per-listing operation against `PRD-186.a`; fixed in code, no migration. `GAP_ANALYSIS.md` v2.60.0, `LISTINGS_SCREEN_CONTRACT.md` v1.6.0 `LSC-054`; `FRAME 20` still blocked**) · **Amended:** 2026-08-18 (**`DARAZ_PROVIDER_CONTRACT.md` v1.6.1 — `DZC-031.h` bounded generic attributes, from the first live pull**) · **Amended:** 2026-08-18 (**`DARAZ_PROVIDER_CONTRACT.md` v1.6.0 — `DZC-031` reported stock is SKU `quantity`; inventory containers not mapped**) · **Amended:** 2026-08-18 (**first production Daraz listing adapter — `LISTINGS_SCREEN_CONTRACT.md` v1.5.0 `LSC-052`, `DARAZ_PROVIDER_CONTRACT.md` v1.5.0 §9 clarifications, `GAP_ANALYSIS.md` v2.59.0; read half only, nothing pulled**) · **Amended:** 2026-08-18 (**`DARAZ_PROVIDER_CONTRACT.md` v1.4.0 — §9 listing-read protocol `DZC-020`–`DZC-030`; `GAP_ANALYSIS.md` v2.58.0 points `GAP-133` at it**) · **Amended:** 2026-08-18 (**integration status corrected — [`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md) v1.4.1 `LSC-040` coverage range, [`GAP_ANALYSIS.md`](GAP_ANALYSIS.md) v2.57.0 `GAP-133` lead-in; row 13 was also stale at v2.55.0. Status rows only, no rule changed**) · **Amended:** 2026-08-18 (**[`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md) v1.4.0 — `FRAME 22` implemented, completing the local-only Listings frames; status row only, rule range unchanged**) · **Amended:** 2026-08-18 (**[`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md) v1.3.0 — `FRAME 21` implemented; status row only, rule range unchanged**) · **Amended:** 2026-08-18 (**[`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md) v1.2.0 — `FRAME 17` accepted as implemented and `LSC-050.b` pinned; status row only, rule range unchanged**) · **Amended:** 2026-08-18 (**`DOC-092` — [`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md) v1.0.0 registered, owning the `LSC-` prefix; the approved Listings Feature Pack tracked under [`design-reference/`](design-reference/README.md); **`GAP-133` connection half recorded closed — `GAP_ANALYSIS.md` v2.56.0**) · **Amended:** 2026-08-17 (`PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.5.0 — `DEP-124`, range now `DEP-001`–`DEP-124`) · **Amended:** 2026-08-17 (**`V14` verified against PostgreSQL 18.6** — `GAP_ANALYSIS.md` v2.55.0) · **Amended:** 2026-08-16 (**`V14` credential persistence** — `TECHNOLOGY_ARCHITECTURE.md` v1.4.0 `TEC-119`/`TEC-120`, `PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.4.0 `DEP-123`, `GAP_ANALYSIS.md` v2.54.0) · **Amended:** 2026-08-16 (`PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.3.0 — `DEP-122`, range now `DEP-001`–`DEP-122`) · **Amended:** 2026-08-16 (`PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.2.0 — `DEP-121`, range now `DEP-001`–`DEP-121`) · **Amended:** 2026-08-16 (stale `GAP-120` references reconciled — `PRODUCTION_DEPLOYMENT_RUNBOOK.md` v1.1.0, `TECHNOLOGY_ARCHITECTURE.md` v1.3.1) · **Amended:** 2026-08-16 (**`GAP-120` CLOSED — production first Owner created and verified**; `GAP_ANALYSIS.md` v2.53.0) · **Amended:** 2026-08-16 (**`AGV-042` initial Owner bootstrap**; `ACCESS_GOVERNANCE_ARCHITECTURE.md` v1.2.0) · **Amended:** 2026-08-16 (`DOC-091` `PRODUCTION_DEPLOYMENT_RUNBOOK.md` registered — first operations authority in the corpus; `TECHNOLOGY_ARCHITECTURE.md` v1.3.0, `TEC-115` amended) · **Amended:** 2026-08-15 (`DOC-090` Screen Contract v2.0.0 — re-audit simplification, 6 surfaces → 3) · **Amended:** 2026-08-15 (`DOC-090` Screen Contract v1.1.0 closure; `DOMAIN_MODEL.md` v3.33.0) · **Amended:** 2026-08-15 (`DOC-090` Shops & Channels Screen Contract registered — first Screen Contract in the corpus) · **Amended:** 2026-08-13 (`DOC-089` Connected Listings reconciliation; `E-106`-`E-108`) · **Amended:** 2026-08-13 (`DOC-088` Product commercial content and media; `E-105` Media Asset) · **Amended:** 2026-08-12 (Final global UI delta; Final global UI foundation; Inventory navigation terminology and Stock Control IA; `PRD-162` Channel Listing capability codes) · **Amended:** 2026-08-11 (`DOC-085` assembled finished-variant identity) · **Amended:** 2026-08-09 (**file reconstructed after data loss — see Appendix C 1.23.1**; **state-machine ratification — `BR-142`**; **pre-freeze GAP and cross-document reconciliation**; `DOC-061`) · **Amended:** 2026-08-08 (Sales reconciliation; immutability `BD-254`; serial policy `BD-242`; Warehouse & Assembly §17; Purchase & Supplier §18; Accounting §19; **documentation consistency pass**) · **Last verified against `/docs`:** 2026-08-12
...
|---|---|---|---|
| 1 | `MASTER_DOCUMENTATION_INDEX.md` | 🧊 **Under freeze governance** — current v1.82.0 | `DOC-001`–`DOC-094` (`DOC-059` withdrawn) |
...
| 4 | [`ORDER_MANAGEMENT_ARCHITECTURE.md`](ORDER_MANAGEMENT_ARCHITECTURE.md) | ✅ **Ratified** v1.21.0 | `BR-001`–`BR-183` |
...
| 7 | [`PERMISSION_ARCHITECTURE.md`](PERMISSION_ARCHITECTURE.md) | ✅ **Ratified** v1.14.0 | `PRM-001`–`PRM-091` |
| 8 | [`AUDIT_ARCHITECTURE.md`](AUDIT_ARCHITECTURE.md) | ✅ **Ratified** v1.6.0 | `AUD-001`–`AUD-043` |
| 9 | [`PRODUCT_ARCHITECTURE.md`](PRODUCT_ARCHITECTURE.md) | ✅ **Ratified** v1.30.0 | `PRD-001`–`PRD-205` |
...
| 13 | [`GAP_ANALYSIS.md`](GAP_ANALYSIS.md) | ✅ **Current** v2.68.0 | `GAP-001`–`GAP-137` |
...
| 37 | [`PRODUCTION_DEPLOYMENT_RUNBOOK.md`](PRODUCTION_DEPLOYMENT_RUNBOOK.md) | ✅ **Ratified** v1.7.0 — 2026-08-23 (`DOC-091`) | `DEP-001`–`DEP-126` |
| 38 | [`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md) | ✅ **Ratified** v1.16.0 — 2026-08-21 (`DOC-092`) | `LSC-001`–`LSC-063` |
| 39 | [`LISTINGS_PAUSE_HANDOFF.md`](LISTINGS_PAUSE_HANDOFF.md) | 📌 **Status record — NOT canonical** v1.1.0 — 2026-08-23 (`DOC-093`) | **— issues no rules** |
| 40 | [`ORDERS_SCREEN_CONTRACT.md`](ORDERS_SCREEN_CONTRACT.md) | ✅ **Ratified** v1.9.0 — 2026-08-24 (`DOC-094`) | `OSC-001`–`OSC-070` |
| 41 | [`STEADFAST_PROVIDER_CONTRACT.md`](STEADFAST_PROVIDER_CONTRACT.md) | ✅ **Implementation reference — NOT canonical** v1.0.0 — 2026-08-24 (`DOC-095`) | `STF-001`–`STF-012` |
| 42 | [`ORDER_MODULE_ROADMAP.md`](ORDER_MODULE_ROADMAP.md) | 📌 **Working record — NOT canonical** v1.0.0 — 2026-08-24 (`DOC-096`) | **— issues no rules** |
| 43 | [`ORDER_MODULE_PAGE_CONTRACT.md`](ORDER_MODULE_PAGE_CONTRACT.md) | 📌 **Working record — NOT canonical** v1.0.0 — 2026-08-24 (`DOC-097`) | **— issues no rules** |
...
```mermaid
flowchart TD
    IDX[MASTER_DOCUMENTATION_INDEX]:::idx

    DR[design-reference images]:::done
    DC[DESIGN_CONSTITUTION]:::done
    OM[ORDER_MANAGEMENT_ARCHITECTURE]:::done
    SY[SYSTEM_ARCHITECTURE]:::done
    DB[DATABASE_ARCHITECTURE]:::done
    PR[PERMISSION_ARCHITECTURE]:::done
    AU[AUDIT_ARCHITECTURE]:::done
    ...
    PC[PROCUREMENT]:::done
    DL[DELIVERY]:::done
    RE[RETURN_EXCHANGE]:::done
    PY[PAYMENT]:::done
    AC[ACCOUNTING]:::done
    AP[API]:::done
    NT[NOTIFICATION]:::done
    RP[REPORTING]:::done
    ...
    classDef idx fill:#FF7A00,stroke:#B35A00,color:#1F1200,stroke-width:2px
    classDef done fill:#E9F7F0,stroke:#0E7C4A,color:#1F2937
    classDef plan fill:#F4F5F7,stroke:#9AA1AC,color:#5B6472,stroke-dasharray:4 3
```
...
> **DOC-093 — 📌 [`LISTINGS_PAUSE_HANDOFF.md`](LISTINGS_PAUSE_HANDOFF.md) IS REGISTERED AS A STATUS RECORD, AND IS NOT CANONICAL ARCHITECTURE** (v1.0.0, 2026-08-22). 🔴 **IT CLAIMS NO RULE PREFIX AND ISSUES NO RULES, DELIBERATELY.** ⚠ **It is registered so it is DISCOVERABLE, not so it is AUTHORITATIVE** — nothing may be cited from it as a rule, and where it appears to state something new, the owning document wins (`DOC-005`, `DOC-010`).
>
> ✅ **WHAT IT RECORDS:** **the state Listings was left in on 2026-08-22**, when the product owner paused the Listing View visual redesign and work moved to the Order module — **completed work** (Daraz connection, the first pull, the accepted `DZC-041` write probe, backend `sale_price`/`listing_stock` push support, the `PRD-204` View and Edit rebuilds, and the `FRAME` register), **paused work**, **six open decisions with their owners**, **the production state**, and **why the redesign stopped**.
>
> 🔴 **IT CHANGES NO STATUS AND CLOSES NO GAP.** **`GAP-134`, `GAP-136`, `DZC-026`/`PRD-202`, `PRD-199` and `PRD-187` are exactly as their owners left them**, and **`LSC-060`'s registration of the *Product Listing* composition refinement stands unamended** — the pause is a work decision, not a documentation one.
>
> ⚠ **IT RECORDS A CONTRADICTION RATHER THAN RESOLVING IT:** **`V15__channel_listing_review.sql` is present in the deployed commit's tree and `DEP-071` applies migrations at backend startup, while `GAP-136` records `V15` as unapplied in production.** ✅ **Resolution is a read of `flyway_schema_history` (`DEP-070.b`, `DEP-031`), never an assumption, and it is owed BEFORE the next backend deployment.**
>
> ✅ **No other document is amended by its admission**, and the three staleness observations it carries — a `DarazChannelAdapter` Javadoc, `DEP-070.a`/`.e`, and this index's own dashboard row 1 — are **reported there and corrected nowhere**, awaiting their owners.

---

> **DOC-094 — ✅ [`ORDERS_SCREEN_CONTRACT.md`](ORDERS_SCREEN_CONTRACT.md) IS WRITTEN, CANONICAL AND REGISTERED** (v1.0.0, 2026-08-23), **owning the `OSC-` prefix.** **It is the THIRD screen contract in the corpus and follows [`LISTINGS_SCREEN_CONTRACT.md`](LISTINGS_SCREEN_CONTRACT.md)'s structure.** 🔴 **The `OSC-` prefix is permanent and is never reassigned** (`DOC-009`, `DOC-013`).
>
> ✅ **IT REGISTERS AN APPROVED DESIGN AS AUTHORITY** — the *Trioloo Orders Screens* artifact and its two artboards, `Order Dashboard` and `Order Detail` (`OSC-010`). **It records the `FRAME 01`–`FRAME 09` register with NONE built**, and fixes per frame what must be VISIBLE, what may be DONE, what must be REFUSED, which rules govern, and what test coverage is owed.
>
> 🔴 **IT CREATES NO BUSINESS RULE, ENTITY, PERMISSION, ENDPOINT, MIGRATION OR VISUAL DECISION.** **Every obligation traces to `BR-001`–`BR-176`, `E-031`–`E-034`, `SM-1`–`SM-11`, `EVT-001`–`EVT-023`, `UX-187` and the `DESIGN_CONSTITUTION.md` token matrix.** ⚠ **Its governing principle is that A SPACE IN THE DESIGN IS NOT A RATIFIED BEHAVIOUR** — **ten blocked elements are registered at `OSC-050` and NONE is decided** (`GAP-004`, `GAP-017`, `GAP-019` residual, `GAP-020`, `GAP-022`–`GAP-025`, `GAP-034`, `GAP-066`).
>
> ⚠ **IT RECORDS THREE UNCOMFORTABLE FACTS RATHER THAN LEAVING THEM IN A REVIEW.** **`OSC-011` — the pack is NOT yet tracked repo-locally, while both existing screen contracts track theirs.** **`OSC-012` — `Order Dashboard.dc.html` and `Order Details.dc.html`, at design-reference precedence 2 and 3, were UNREACHABLE (`HTTP 404`), did not inform the artboards, and OUTRANK them if they return.** **`OSC-052` — no `order.*` permission code is ratified, implementation may never coin one (`PRM-089.f`, `PRM-003`), and this is the hardest blocker on any Orders slice.**
>
> 🔴 **IT PRESERVES THE `V15` PRODUCTION CONTRADICTION AND FORBIDS PROPOSING ANY MIGRATION NUMBER UNTIL PRODUCTION `flyway_schema_history` IS READ** (`OSC-060`, `DEP-070.b`, [`LISTINGS_PAUSE_HANDOFF.md`](LISTINGS_PAUSE_HANDOFF.md) §4).
>
> ✅ **No other document is amended by its admission.** ⚠ **`§5.4`'s prefix registry is NOT updated here — it already omits `SCS-`, `LSC-`, `DEP-`, `UX-`, `PRJ-` and `TEC-`, so retrofitting one entry would misrepresent it as current; the staleness is reported for this document's own owner.**
