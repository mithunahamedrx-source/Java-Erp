...
> **`OSC-059` — ✅ THE INVOICE VISUAL AUTHORITY, AND THE `E-039` SNAPSHOT. 2026-08-24.**
>
> | | |
> |---|---|
> | **Artifact** | *TrioLoo Invoice* — `design-reference/TrioLoo Invoice.html` |
> | **Specified at** | [`design-reference/TrioLoo Invoice.md`](design-reference/TrioLoo%20Invoice.md) |
> | **Record** | `sales_invoice` (`V22`) — `E-039` |
>
> **a.** ✅ **`OSC-058.b`'s BLOCK ON `Print` IS HALF DISCHARGED.** **It refused the action because
> `PRN-023` sources the printable from an `E-039` record whose content `INV-39.2` requires
> snapshotted, and none existed.** ✅ **`V22` creates it and `SalesInvoiceService` fills it.**
> 🔴 **THE OTHER HALF STANDS: no renderer exists, and `GAP-003` still supplies no tax rate.**
> **The `Print` control therefore stays disabled.**
> **b.** 🔴 **THE INVOICE ADOPTS THE ORDER'S NUMBER AND MINTS NONE.** **`OSC-057` assigned it and
> `V19`'s trigger made it immutable.** ⚠ **Issuing a second number here would be the second
> sequence `BD-443` and `INV-39.1` prohibit outright.**
> **c.** 🔴 **THE CONTENT IS A SNAPSHOT AND NOT A VIEW** (`INV-39.2`, `PRN-022`). **Customer name,
> phone, address, every line name and price, and all totals are COPIED at issue.** ⚠ **A renderer
> that re-read the order would reprint last year's invoice with this year's address and this year's
> prices — which is precisely what `INV-39.2`'s *"reproducible years later"* exists to prevent.**
> ✅ **A test changes the order after issue and asserts the invoice does not move.**
> **d.** 🔴 **THE DESIGN'S `vatRate = 7.5` IS SAMPLE DATA AND IS NOT IMPLEMENTED.** ⚠ **It is
> hard-coded in the mock's `renderVals()` BESIDE its sample Lenovo and Samsung line items and a
> sample `charges = 800` — the same block and the same status.** 🔴 **`GAP-003` records that NO tax
> model exists: no rate, no BIN, no Mushak requirement, no calculation, and Bangladesh has more
> than one lawful rate — which applies depends on Trioloo's registration and product categories,
> which is a LEGAL question.**
> **d.i.** ✅ **THE FIELD IS LEGITIMATE AND THE NUMBER IS NOT.** **The product owner ratified that
> the invoice CARRIES VAT (2026-08-24), and `BD-307` permits VAT to be DISPLAYED as an invoice
> field while the ERP maintains no VAT accounts.**
> **d.ii.** ✅ **THE RATE IS RATIFIED AT `0%`, 2026-08-24 — AND EXPLICITLY *FOR NOW*.**
> 🔴 **ZERO IS A RATE, NOT AN ABSENCE, AND THE DIFFERENCE IS THE POINT.** **An UNSET value records
> `NULL` and says nobody has decided; `0.000` STATES that no tax applies at present** (`SYS-034`).
> ⚠ **The owner has decided, so the invoice carries the claim** — which is exactly what `BD-307`
> permits displaying while the ERP maintains no VAT accounts. **`V22`'s CHECK keeps the rate and
> the amount stated together — a rate without an amount is a half-stated tax fact.**
> **d.iii.** 🔴 **`GAP-003` IS NOT CLOSED BY THIS.** **No BIN, no Mushak requirement and no rule
> about which rate applies to which product category exists.** ⚠ **What is ratified is Trioloo's
> CURRENT POSITION, not a tax model.**
> **d.iv.** ✅ **A LATER RATE CHANGE CANNOT REWRITE AN INVOICE ALREADY ISSUED, AND THIS IS WHY
> `INV-39.2` MATTERS RATHER THAN BEING PAPERWORK.** **The rate and the amount are COLUMNS on the
> issued document, not a calculation performed at render time** (`PRN-022`). 🔴 **A renderer that
> re-derived tax from current configuration would silently restate every historical invoice at the
> new rate — wrong for the customer, and the kind of thing a tax authority reads as
> falsification.** ✅ **A test pins it: two invoices, two configured rates, and the first stays at
> `0%`.**
> **e.** ✅ **THE OWNER'S REFERENCE ORDERING IS IMPLEMENTED**: the Trioloo invoice number is the
> IDENTITY, and the courier booking id and the marketplace order number are REFERENCES stored
> after it. 🔴 **Each keeps its ISSUING PARTY in its own column** (`DB-013`) — ⚠ **the design
> labels one `Parcel ID.` with no issuer, which is the ambiguity `Phase 4` removed from the Order
> Card.**
> **f.** ✅ **THE DESIGN SUPPLIES REAL TRIOLOO DATA — address, three phone numbers, two emails,
> the Al-Arafah bank account and the website — as LITERAL TEXT rather than placeholders**, which
> is how it distinguishes them from sample values. ⚠ **They are nonetheless CONFIGURATION: a bank
> account number compiled into a printable is changed by a deployment, and the same number in
> configuration is changed by the person who owns it.**
> **g.** ⚠ **FOUR MORE SAMPLE VALUES ARE REFUSED, EACH RECORDED IN THE SPECIFICATION**: the four
> demo line items; `charges = 800` (`DLV §13` owns the customer delivery charge); `Collected
> Advance`, **which is exactly the pre-delivery partial payment `GAP-035` leaves open and `§11.3`'s
> `NOT_DUE` argues against**; and a `Due` date, **which implies a payment term no rule in the
> corpus states**.
> **h.** ⚠ **THE DESIGN'S TWO STATUS ENUMS ARE NOT CANONICAL STATES.** **`Paid`/`Partially
> Paid`/`Due` is not `SM-5`, whose eleven states `OSC-056.b` already narrowed to the three this
> system can honestly show; `Delivered`/`Shipped`/`Processing`/`Pending` is not `SM-4`, and
> `Shipped` and `Processing` are not `SM-4` states at all.**
> **i.** ✅ **THE COMPOSITION IS KEPT IN FULL** — the A4 sheet at `794×1123`, the black-header items
> table, the quiet subtotal/charges/VAT rows against the ink-filled `Balance Due` block, the bank
> panel, and the two status chips kept SEPARATE, which is `BR-171`/`UX-182`'s own principle
> arriving independently.
> **j.** ⚠ **THE DOCUMENT TYPEFACE IS `Hanken Grotesk` / `Space Grotesk`, NOT `Manrope`.**
> **`DESIGN_CONSTITUTION.md` fixes Manrope for the APPLICATION UI; `DOCUMENT_ARCHITECTURE.md` §15
> decides no typography for printables at all.** ✅ **So the document face was genuinely undecided
> and this design is the first proposal for it — recorded rather than assumed ratified.**
...
| Version | Date | Change |
|---|---|---|
| **1.11.0** | **2026-08-24** | ✅ **THE INVOICE VISUAL AUTHORITY AND THE `E-039` SNAPSHOT — `OSC-059`.** ✅ **`V22` creates the `sales_invoice` record `INV-39.2` requires, and `SalesInvoiceService` SNAPSHOTS the content — customer, address, every line name and price, all totals — so the document reproduces years later. A test changes the order after issue and asserts the invoice does not move.** ✅ **This half discharges `OSC-058.b`'s block on `Print`**; 🔴 **the other half stands — no renderer exists and `GAP-003` supplies no tax rate — so the control stays disabled. 🔴 THE DESIGN'S `vatRate = 7.5` IS SAMPLE DATA AND IS REFUSED: it is hard-coded beside the mock's sample Lenovo and Samsung line items and a sample `charges = 800`. `BD-307` makes the VAT FIELD legitimate; the NUMBER is a legal question about Trioloo's registration, and unset records NULL rather than a `0%` claim (`SYS-034`).** ✅ **The invoice ADOPTS the order's number and mints none — a second sequence is prohibited outright (`BD-443`, `INV-39.1`).** ✅ **The owner's reference ordering is implemented, each with its ISSUING PARTY in its own column (`DB-013`).** ⚠ **Four further sample values refused, and the design's two status enums are recorded as NOT `SM-4`/`SM-5`.** |
| **1.10.0** | **2026-08-24** | ✅ **`OSC-030.a` GAINS `Courier booked`.** **`COURIER_BOOKED` was ratified into `SM-1` on 2026-08-24 resolving `GAP-139` — `OM §6.2` had carried it since v1.1.0 while `SMA §5.2` had not, and `BR-082` depended on it to place the amendment boundary STRICTER than `BR-011`.** ⚠ **The tab can only be empty until a booking path ships, which is an honest fact rather than a defect — the same reading `OSC-054` already applies to `CONFIRMED`, `RELEASED`, `IN_FULFILLMENT`, `ON_HOLD` and `CLOSED`.** 🔴 **No channel order ever ARRIVES in this state: booking is a Trioloo-internal Warehouse act, so no adapter translation reaches it.** |
