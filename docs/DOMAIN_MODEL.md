...
| | |
|---|---|
| **Purpose** | **The canonical operational sales / integration channel KIND of a Channel Instance** — a behavioural category of order source, which MAY ALSO identify a distinct adapter family |
...
**The recognised set** — **Daraz · Website · Shopify · WooCommerce · Facebook · WhatsApp · Phone · Walk-in.**

> ⚠ **REFINED 2026-08-15; the superseded Purpose and the earlier six-type list are retained above and here** (`DOC-009`): *"A category of order source, defined by behavioural attributes rather than identity"* · *"Types in operation — Daraz · Website · Facebook · WhatsApp · Phone · Walk-in."*
>
> **The refinement was forced by a real case, not by tidiness.** **A Shopify store, a WooCommerce store and a bespoke website are IDENTICAL on all four `OM §3.1` axes — own storefront, own customer, own fulfilment, direct settlement — yet they cannot share an integration: their external contracts differ entirely.** ✅ **Channel Type is therefore refined to be the channel KIND, which carries behaviour AND may distinguish an adapter family.**
>
> 🔴 **NO NEW ENTITY IS CREATED.** **No Provider, Platform, Marketplace Account or Seller Account concept is introduced** (`DM-084.a`): **the existing `E-015` / `E-016` pair already expresses this, and adding a parallel identity would be the defect `INV-16.11` forbids.**
...
- `INV-15.3` **THE BOUNDARY BETWEEN ADAPTER ROUTING AND DOMAIN BRANCHING. Sharpened 2026-08-15 — `INV-15.1` is NOT weakened.** ✅ **PERMITTED: the INTEGRATION layer resolving which adapter serves a Channel Instance from its canonical Channel Type** (`INV-15.2`, `API-071.a`). ⚠ **That is routing — choosing which implementation speaks the remote protocol — and it is precisely what `INV-15.2` already confines to adapters.** 🔴 **FORBIDDEN: DOMAIN OR BUSINESS CODE DERIVING BEHAVIOUR FROM A RAW CHANNEL TYPE COMPARISON** — `if channelType == "SHOPIFY"`, `if channelType == "WOOCOMMERCE"` — **unless a canonically defined rule genuinely depends on that Channel Type.** ⚠ **Business behaviour comes from declared capabilities, contracts and ratified rules** (`PRD-125`, `API-063`), **never from a string the core happens to hold.** 🔴 **The refinement of the Purpose above widens what a Channel Type may IDENTIFY; it widens nothing about what the domain may BRANCH ON.**
- `INV-15.4` **THE SET IS CLOSED, AND THE EXPOSED SUBSET NEED NOT EQUAL IT. Ratified 2026-08-15.** 🔴 **A Channel Type is always chosen from the recognised set; FREE TEXT IS FORBIDDEN**, because adapter resolution reads this value and an unrecognised one yields a Channel Instance that can never resolve its adapter. ✅ **A surface MAY expose a SUBSET** — the Shops & Channels registry offers the listing-capable types and omits the manual channels, which carry no listings (`PRD-028`). ⚠ **MEMBERSHIP OF THE SET IMPLIES NEITHER THAT AN ADAPTER EXISTS FOR THAT TYPE NOR THAT EVERY FEATURE OFFERS IT.**
- `INV-15.5` **THE THREE FIRST-PARTY WEB TYPES ARE DISTINCT KINDS, NOT DUPLICATES. Ratified 2026-08-15.** ✅ **`Website`** — **the generic or bespoke first-party web sales channel, where no platform-specific adapter family applies.** ✅ **`Shopify`** — **a Shopify-backed web sales channel and adapter family.** ✅ **`WooCommerce`** — **a WooCommerce-backed web sales channel and adapter family.** ✅ **`Daraz` remains the Daraz marketplace channel and adapter family.** ⚠ **The three web types may share most ordinary business behaviour and still be different Channel Types, because their EXTERNAL INTEGRATION CONTRACTS differ** — **which is exactly the distinction `INV-15.3` permits.**
...
**Key attributes** — name; channel type; business unit; **market**; **internal code**; **external shop identifier**; **external link**; **binding, authorisation and activation facts**; **commission structure (versioned)**; settlement cycle; default warehouse; courier preference; credentials reference.
...
- `INV-16.7.a` **THE MARKET SET IS CLOSED AND ERP-SUPPLIED. Ratified 2026-08-15.** 🔴 **A Market is SELECTED from the recognised set; FREE TEXT IS FORBIDDEN**, and an unrecognised value is REJECTED rather than stored. 🔴 **ARBITRARY TEXT IS NEVER NORMALISED ONTO A RECOGNISED MEMBER** — quietly mapping a mistyped value onto one would turn an operator's error into a business fact and would hide the day a further market is genuinely needed.
- `INV-16.7.b` **THE CURRENT SET HAS EXACTLY ONE MEMBER. Ratified 2026-08-15.**

  | Canonical value | Display label |
  |---|---|
  | **`BANGLADESH`** | **Bangladesh** |

  ✅ **That is the ratified set, NOT a placeholder** — it is the only value the approved Shops & Channels design shows, in Add Shop, in the Edit fixed state and on Shop Detail alike. 🔴 **`India`, `Pakistan`, `United States`, `Global`, `International`, `Asia` AND EVERY OTHER PLAUSIBLE VALUE ARE ABSENT DELIBERATELY.** ⚠ **A further member is added by CANONICAL AMENDMENT, never by an implementation choice.**
- `INV-16.7.c` **MARKET IS NOT THE CHANNEL TYPE, AND NEITHER IS DERIVED FROM THE OTHER** (`E-015`). ✅ **A Daraz shop and a Shopify shop may both operate in `BANGLADESH`; the two facts stay separate and are stored separately.**
- `INV-16.7.d` **MUTABILITY FOLLOWS THE BINDING, exactly as the channel type does** (`SCS-030`). ✅ **Selected at registration; still changeable while no external account is bound; 🔴 FIXED once an account is bound**, because an account belongs to the market it was opened in. ⚠ **A Channel Instance that predates this ratification may hold NO market at all; that is a genuine absence** (`SYS-034`) **and is NEVER back-filled to a recognised member, which would fabricate an attribution nobody made.**
...
- `INV-16.14` **THE EXTERNAL LINK IS A SECOND REMOTE FACT AND IS NEVER THE IDENTITY. Ratified 2026-08-15.** ✅ **A Channel Instance MAY carry an operator-facing address at which its account or storefront can be opened on the channel.** 🔴 **IT IS NOT THE EXTERNAL SHOP IDENTIFIER, IS NEVER USED FOR BINDING OR FOR THE `INV-16.6` MISMATCH TEST, AND THE TWO ARE NEVER COLLAPSED.** ⚠ **It is REMOTE-DERIVED and never typed** (`INV-16.5`), **is OPTIONAL because not every channel exposes one, and is absent until an account is bound.** ✅ **It is a safe, non-secret business fact** (`INV-16.8`).
- `INV-16.15` **THE BINDING AND ACTIVATION FACTS ARE FIRST-CLASS AND ARE CAPTURED WHEN THE ACT OCCURS. Ratified 2026-08-15.** ✅ **WHEN the instance became bound to its current exact account · WHEN its authorisation was last successfully established or renewed · WHEN `DRAFT → ACTIVE` succeeded · WHICH ACTOR performed that transition.** 🔴 **BINDING TIME AND AUTHORISATION TIME ARE DIFFERENT FACTS:** **re-authorising the same account renews the authorisation and does NOT re-bind** (`INV-16.6`), **so the binding time is stable across renewals.** 🔴 **These are recorded at the moment of the authoritative act and are NEVER reconstructed from logs or inferred at render time.**
- `INV-16.16` **A CONNECTION CONDITION HAS AN OBSERVATION TIME, AND AN UNREAD CONDITION IS NOT A CONDITION. Ratified 2026-08-15.** ✅ **Where a connection condition is shown, the time it was last ACTUALLY OBSERVED may be shown with it.** 🔴 **A PAGE LOAD IS NOT AN OBSERVATION.** ⚠ **Where the condition cannot be read, the system states that it does not know rather than claiming one** (`SYS-034`), **and the Channel Instance's own record remains fully valid** (`API-069`).
